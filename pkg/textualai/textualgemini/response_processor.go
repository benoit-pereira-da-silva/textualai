...
type ResponseProcessor[S textual.Carrier[S]] struct {
	...
	// Model is the Gemini model identifier.
	//
	// You can pass:
	//   - "gemini-2.5-flash" (will be normalized to "models/gemini-2.5-flash")
	//   - "models/gemini-2.5-flash"
	//   - "tunedModels/..." (left as-is)
	Model string `json:"model,omitempty"`

	// Template is the prompt template used to build the message text.
	...
	// Role is used when the processor constructs a default content item.
...
func NewResponseProcessor[S textual.Carrier[S]](model, templateStr string) (*ResponseProcessor[S], error) {
	...
	tmpl, err := template.New("gemini").Parse(templateStr)
	if err != nil {
